...
除了 Makefile 指令，也可以透過 Claude Code 的 Skills 來操作：

### `/new-post` - 新增文章

**觸發詞：** `new post`、`新文章`、`新增文章`

**功能：**
- 詢問文章主題，自動生成暫定標題和檔名
- 詢問 tags（可選）
- 建立 markdown 檔案，預設 `draft: true`
- 自動填入 `author: davidlei`
...
│  3. 準備發布                                             │
│     > 將 draft: false                                    │
│     > 補上 description（deploy 時會提醒）                 │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│  4. 部署                                                 │
│     > "deploy" 或 "/deploy"                              │
│     > 自動更新發布時間、執行 make deploy                  │
│     > 網站：https://davidleitw.github.io                 │
└─────────────────────────────────────────────────────────┘
```
