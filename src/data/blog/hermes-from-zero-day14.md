...
Hermes 的 plugin 系統(Day 10 講過)用 hook 機制——`on_message`、`on_tool_call`、`on_error`(hook = 預留的「掛鉤點」,在 agent 跑到特定事件時自動呼叫所有註冊的 plugin 回呼)。漂亮。但去看 hook 被觸發的程式碼——這是個典型的 fire-and-forget(發出去就不管)寫法:
