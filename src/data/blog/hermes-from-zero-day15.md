...
Day 14 講過 Hermes 最深的結構性債務:`run_conversation()` 是一個約 3,900 行的單一函式(整檔 4,099 行)。團隊**知道**模組化重要,他們把方法 body 推進了 `agent/` 套件——但他們**停在「搬移程式碼」,沒有走到「定義介面 + 狀態所有權」**。
...
「保留」是抄 Hermes 做對的;「修掉」是繞開 Hermes 做歪的。但還有一塊 Hermes 根本沒做——這 14 天讀下來,我覺得從第一天就該補上的東西。
