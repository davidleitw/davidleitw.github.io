...
14 天前,你打開一個空白檔案,寫下 `from openai import OpenAI`,然後想:「我要做一個 agent」。你腦袋裡的東西是這樣的——`client.chat.completions.create(messages=[...])`、`tools=[...]`、一個 while loop、收到 `finish_reason='tool_calls'` 就執行工具、塞回去、再呼叫一次。聽起來很簡單。
...
昨天我們把 Hermes 的瑕疵攤開來——巨石檔案、24 種英文字串比對、`len // 4` 的中文災難、沒有 agent eval、計數器混亂、並行工具不擋 implicit shared state、plugin hook 默默吞 exception、aux LLM 掛掉時的 silent data loss。今天不是再批一次。今天是把這 14 天看到的東西收成一份「**如果是我,我會怎麼做**」。
...
這條是暗線 A 在 Day 2、5、8、9、12 反覆出現的東西。`AIAgent` 不知道是誰在呼叫它。它接受 user input、跑迴圈、產 output、把事件 yield 出來——至於呼叫它的是 CLI、是 gateway HTTP、是 MCP server、是 ACP adapter、還是 cron scheduler,它都不在乎。
...
這看起來只是個 logging 細節。它其實是可觀測性的紀律——當你的 agent 在生產環境跑了一個月,使用者抱怨「它有時候會卡住」,你打開 log,你立刻知道每一次卡住是哪一種卡住。沒有這個 enum,你就在大海撈針。

每個結束點都要有名字。這條跟「每個錯誤都要有 enum」(Hermes 的 `FailoverReason` 18 種值)是同一條紀律。

### 4. 委派 = context 防火牆

Day 11 講子代理委派。表面上看,子代理是「叫人幫忙」——「你去幫我搜尋一下這個」、「你去幫我讀那個檔案」。但 Hermes 的設計暗示了另一件事:**子代理的本質是「建立一道 context 邊界」**。
...
這個設計**讓謊報完成在結構上不可能**。用一個外部計數器+結構性追加,讓謊報這件事在下一輪就被打臉。Model 看到自己上一輪的承諾跟現在的計數器衝突,自然就轉向。
...
上面那些是「值得偷的」。現在翻另一面——同一個 codebase,有些地方我會刻意不照抄。下面是 Day 14 攤開來的問題,我自己刻的時候會從第一天就避開的東西。

### 1. 抽到底——不要停在「搬移程式碼」

Day 14 講過 Hermes 最深的結構性債務:`run_conversation()` 是一個約 3,900 行的單一函式(整檔 4,099 行)。團隊把方法 body 推進了 `agent/` 套件——但**停在「搬移程式碼」,沒有走到「定義介面 + 狀態所有權」**。
...
前面講了「保留」跟「修掉」。還有一塊 Hermes 根本沒做——這 14 天讀下來,我覺得從第一天就該補上的東西。
...
這 14 天讓你看見一個 production agent framework 的全貌——包含它的優雅、它的疤痕、它的歷史包袱。
...
**寫 agent 的人很多,讀 framework 的人很少**。如果你撐到了 Day 15,你已經跟大部分 agent 開發者不一樣了——你已經「看得懂一個 agent framework 為什麼長這樣、哪裡長歪了」。這個能力比任何一個框架都活得久。
...
1. **去 [Hermes 的 docs](https://hermes-agent.nousresearch.com/docs/) 看怎麼真的架一台來用**——這個系列是「怎麼長出來的」,docs 是「怎麼用它」,互補。
...
| Day | 主題 | 主要原始碼入口 |
|---|---|---|
| 02 | 核心迴圈 | `agent/conversation_loop.py`(整檔 4,099 行,`run_conversation()` 從 line 187 開始,函式本體約 3,900 行)|
| 02 | 結束點 enum | `agent/conversation_loop.py` 找 `_turn_exit_reason` |
| 03 | Prompt cache 規則 | `agent/system_prompt.py`、`agent/prompt_caching.py`、`agent/prompt_builder.py` |
| 04 | Context 壓縮 | `agent/context_compressor.py`、`agent/context_engine.py`、`agent/conversation_compression.py` |
| 05 | Provider 抽象 | `providers/base.py` 的 ABC + `agent/anthropic_adapter.py` / `bedrock_adapter.py` / `gemini_native_adapter.py` / `codex_responses_adapter.py` 等 |
| 06 | 記憶系統 | `agent/memory_manager.py`、`agent/memory_provider.py`、`agent/curator.py`、`agent/curator_backup.py` |
| 07 | 工具系統 | `agent/tool_executor.py`、`agent/tool_guardrails.py`、`tools/` |
| 07 | 並行工具白名單 | `agent/tool_dispatch_helpers.py` 找 `_PARALLEL_SAFE_TOOLS` |
...
| 12 | 狀態持久化 | `hermes_state.py`(整檔 3,238 行,內含 `SessionDB` class 從 line 309 開始,class body 約 2,930 行)|
