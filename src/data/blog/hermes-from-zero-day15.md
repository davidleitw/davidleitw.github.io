...
第三天,你讀到 `system_prompt_session` 不准動那條鐵律,腦袋裡突然「啊!」了一聲——原來那不是性能優化,是整個 API 的形狀都被這條不變式長出來的。第七天,你看到 `tool_guardrails.py` 用三個純函式計數器擋住假完成,你心裡冒出「這個我可以偷」的興奮。第十四天,你看到 `cli.py` 657KB、`run_conversation()` 約 3,900 行的單一函式(整檔 4,099 行),你開始懂為什麼那個系列的最後一篇叫做「自己刻會更好」。
