...
`tests/` 目錄底下有 **1,172 個檔案、約 23,500 個測試函式**。聽起來很多吧?但你打開一看會發現,絕大多數測的都是「**給定一段 fake 的 LLM response,系統會怎麼處理**」,而**不是**「**這個 agent 給定一個任務,能不能完成**」。
...
- `contributor-check.yml`:作者 email 不在 `release.py` 的 `AUTHOR_MAP` 就讓 PR 失敗。
...
但講到這你應該有感覺——這東西**不是完美的**。`cli.py` 657KB、`run_agent.py` 17 萬行、沒有 agent eval、Windows CI 從來沒被執行過。明天我們不再護航,正面開砲——**Hermes 哪裡做得不夠好,如果你要寫自己的 agent,哪些坑你應該繞開**。
...
從 `tests/conftest.py` 開始讀,理解四個 autouse fixture 各自關掉什麼;然後挑一個 `test_*_recovery.py` 看 fake LLM 怎麼構造;最後讀 `scripts/run_tests.sh` 理解本機跟 CI 怎麼對齊。
