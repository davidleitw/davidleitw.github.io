...
驗證器(`tools/skill_manager_tool.py::_validate_frontmatter`)強制 4–5 條硬規則:檔案開頭必須是 `---`、結尾要有對應的 `---`、`name` 必填、`description` 必填且 ≤1024 字元;另外 `_validate_content_size` 限制整個檔案 ≤100,000 字元。其他像 `version`、`author`、`license` 都是慣例,**不強制**。
...
這就讓「一個技能攜帶 100k 字元的酬載,在被呼叫之前只花約 300 token」變成可能(description 上限 1024 字元 ≈ ~300 token)。授權規範甚至明文要求:「如果你的 body 超過 20k 字元,就拆進 `references/*.md`。」
