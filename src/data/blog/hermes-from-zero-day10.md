...
先講 Anthropic 官方 — Agent Skills 於 **2025 年 10 月 16 日** 釋出,官方規格的目錄結構非常單純:
...
**沒有「分類」層**。`references/`、`templates/` 都是社群慣例,不是官方規格。

Hermes 在這上面加了**一層自己的分類目錄**(`skills/<分類>/<名稱>/SKILL.md`)——你可以從 `agent/prompt_builder.py` 裡的 `skills_by_category` / `category_descriptions` 看到。這是 Hermes 自己的設計,不是 Anthropic 規格。

沒有編譯步驟、沒有 plugin API、沒有 DSL。純文字,可 diff、可 review、可 grep。**人跟 agent 用同一套 `Edit` / `Write` 工具就能編輯它**——這是 Anthropic 在 Agent Skills 公告與工程部落格反覆強調的洞見:「With bash and filesystem tools, agents can perform actions just as humans would.」

驗證器(`tools/skill_manager_tool.py::_validate_frontmatter`)強制 4–5 條硬規則:檔案開頭必須是 `---`、結尾要有對應的 `---`、`name` 必填、`description` 必填且 ≤1024 字元;另外 `_validate_content_size` 限制整個檔案 ≤100,000 字元——**這是 Hermes 自己加的保護網**,Anthropic 官方並沒有這個硬上限,官方建議是「body 控制在 5k tokens / 500 行以內,超出就拆到補充 `.md`」。其他像 `version`、`author`、`license` 都是慣例,**不強制**。
...
Hermes 在這層官方規格之上,還加了一層**自己的分類描述層**——在組 system prompt 的 skill index 時,每個分類附上一行 `category_description`。這不是 Anthropic 規格,純粹是 Hermes 為了處理 89 個技能而加的二級路由訊號。你可以把它視為「Hermes 自己的 Level 0.5」,但跟 Anthropic 官方無關。
