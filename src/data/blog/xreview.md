...
xreview 是一個 Claude Code plugin，把 code review 丟給 codex（OpenAI），讓 Claude Code 負責驗證跟執行修正。

流程是一個三方迴圈。codex 獨立讀你的 code，找 bug、安全漏洞、邏輯問題。Claude Code 收到結果之後不是照單全收，而是逐一去讀原始碼確認問題是不是真的存在，遇到可疑的 finding 會主動挑戰 codex。最後所有確認過的問題一次呈現給你，每個都附修復方案，你選要修哪些。

修完之後 codex 再驗證一輪，不 OK 就繼續，最多五輪。結束後自動產一份 markdown 報告。
...
所以後來加了 Claude Code 在中間做過濾。它會實際去讀 codex 指出的那幾行 code，判斷問題是不是真的存在。覺得有疑慮的會跟 codex 來回討論：「你說這邊有 race condition，但我看了 lock 的 scope，這兩個 goroutine 不會同時持有這個 lock，你確定嗎？」
...
xreview 是 Go 寫的 CLI，跑在 Claude Code 跟 codex 之間。一個 binary，不需要額外 runtime。
...
Session 狀態存在 `.xreview/sessions/`，codex 可以 resume 之前的 session，驗證輪不用從頭跑。

安裝的話，在 Claude Code 裡兩行：
...
坦白說 xreview 做的事情很窄。它不管 CI/CD，不管 PR workflow，不管團隊協作。它就做一件事：拿一個不同的模型來看你的 code，然後用 Claude Code 過濾噪音。如果你需要的剛好是這個，它很好用。

接下來想做的是語言特化的 review context。目前 xreview 不管你寫什麼語言都用同一套方式看，但不同語言有不同的地雷區，像 Go 的 error handling 慣例、Rust 的 ownership、C++ 的 lifetime 管理。做法是讓 xreview 在打 codex 之前多帶一段語言對應的 review 指引，核心流程不用改，就是多組一段 prompt 進去。
