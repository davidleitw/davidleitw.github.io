...
好,進到第二層。`ContextCompressor.compress()` 的演算法分四個 phase。重點是它**不是一上來就叫 LLM 摘要**——而是先做一堆不用 LLM 的便宜事。
...
(實際模板有 13 個 section——完整列表見 `context_compressor.py:960–1014` 的 `_template_sections`,這裡節錄四個最重要的。)
